...
def parseLwSw(op, argv):
    ...
    return opcode +'_'+ parseRegister(rs) +'_'+ rt_str +'_'+ num2bin(offset, 16)
...
def parseR(op, argv):
    ...
    return (opcode +'_'+ parseRegister(argv[1]) +'_'+ parseRegister(argv[2])
            +'_'+ parseRegister(argv[0]) +'_'+ '0' * 5 +'_'+ funt)
...
def parseImm(op, argv):
    ...
    return opcode +'_'+ parseRegister(argv[1]) +'_'+ parseRegister(argv[0]) +'_'+ num2bin(argv[2], 16)
...
def parseShift(op, argv):
    ...
    return '0' * 11 +'_'+ parseRegister(argv[1]) +'_'+ parseRegister(argv[0]) +'_'+ num2bin(argv[2], 5) +'_'+ funt
...
def parseBranch(op, argv, labels, cur_addr):
    ...
    if op == 'beq':
        return '000100' +'_'+ rs_str +'_'+ parseRegister(argv[1]) +'_'+ offset_str
    elif op == 'bne':
        return '000101' +'_'+ rs_str +'_'+ parseRegister(argv[1]) +'_'+ offset_str
    elif op == 'blez':
        return '000110' +'_'+ rs_str +'_'+ '0' * 5 +'_'+ offset_str
    elif op == 'bgtz':
        return '000111' +'_'+ rs_str +'_'+ '0' * 5 +'_'+ offset_str
    elif op == 'bgez':
        return '000001' +'_'+ rs_str +'_'+ '00001' +'_'+ offset_str
    else:
...
def parseJump(op, argv, labels):
    '''jump label'''
    if op == 'j':
        return num2bin('0x02', 6) +'_'+ num2bin(str(labels[argv[0]]), 26)
    else:
        return num2bin('0x03', 6) +'_'+ num2bin(str(labels[argv[0]]), 26)
...
filename = input('filename(default=input.txt): ')
if filename == '':
    filename = 'input.txt'
...
lines = [delComment(line) for line in lines if delComment(line) != '']
...
for i in range(len(instructions)):
    bin_str.append(parseInstruction(instructions[i], labels, i))
...
for out in bin_str:
    ...
    print('memory['+ str(i) +'] <= 32\'b' + out + ';')
    f.write('memory['+ str(i) +'] <= 32\'b' + out + ';\n')
    i+=1
